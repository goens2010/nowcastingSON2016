...
public class LuatMoAFC {
	static AFC_PSO afc = new AFC_PSO();
	//private static int cluster = Define.NUM_RULES;
	protected static int cluster = afc.AFC_PSO();;
	...
	public static Pixel[][] training_sple2(int number_image) {
		...
		Pixel[] p1 = new Pixel[Define.IMAGE_SIZE_COL * Define.IMAGE_SIZE_ROW];
		Pixel[] p2 = new Pixel[Define.IMAGE_SIZE_COL * Define.IMAGE_SIZE_ROW];
		...
		while (i < number_image - 1) {
			j = i + 1;
			 p1 = FC_PFS2.readData(filename[i]);
			// System.out.println(p1[10].toString());
			 p2 = FC_PFS2.readData(filename[j]);
	...
	*/
	...
	public static double[][] tinh_Ujt(int number_image){
		double [][] u = LuatMo2.readMatrixU(Define.path + "/matranU_Gbest.txt",number_image);
		double [][] e = LuatMo2.readMatrixU(Define.path + "/matranE_Gbest.txt",number_image);
		//double [][] n = LuatMo2.readMatrixU(Define.path + "/matranN_rule.txt",number_image);
	...
	public static double[][] tinh_a1(ArrayList<double[][][]>list,ArrayList<double[][][]>list_u1,double[][]a){
		double[][] arr = new double[cluster][3];
...
//			if(m1 == 0 || n1 ==0 || p1 ==0 ) System.out.println("division by zero");
	...
	public static double[][] tinh_a(ArrayList<double[][][]>list,ArrayList<double[][][]>list_u1,double[][]b){
		double[][] a1 = new double[cluster][3];
		//double m = 0, n = 0, p = 0, m1 = 0, n1 = 0, p1 = 0;
		for(int i = 0; i < cluster;i++){
			double m = 0, n = 0, p = 0, m1 = 0, n1 = 0, p1 = 0;
			for(int j = 0;j < list.size(); j++){
	...
	public static double[][] DEF(double[][][] a) {
		double[][] def = new double[cluster][3];
		System.out.println("do dai DEF" + a.length + " cluster = " + cluster + " len = " + a[0].length);
		for (int i = 0; i < cluster; i++) {
...
/*public static Pixel [][] forecast( double[][][] output,int number_image) {//Pixel[][] array,
	...
	for(int i = 0; i < Define.IMAGE_SIZE_ROW; i++){
		for(int j = 0; j < Define.IMAGE_SIZE_COL; j++){
			...
			for(int t = 0; t < 3; t++){
               ...
               //Moi bo tg += Math.pow(result[i][j][t]- b[i][j][t], 2);
                                
				//Moi bo tg1 = tg1 + b[i][j][t];
				
				//if(output[i][j][t] != 0)
					//System.out.print("anh goc: "+ a[i][j][t]+" " +" chenh lech: "+ output[i][j][t] + " ");
			}
			/*tg = (result[i][j][0] - Data.getPixel_goc(i, j, 4).getRed()) * (result[i][j][0] - Data.getPixel_goc(i, j, 4).getRed())+
					(result[i][j][1] - Data.getPixel_goc(i, j, 4).getGreen())*(result[i][j][1] - Data.getPixel_goc(i, j, 4).getGreen())+
					(result[i][j][2] - Data.getPixel_goc(i, j, 4).getBlue())*(result[i][j][2] - Data.getPixel_goc(i, j, 4).getBlue());*/
			tg = (result[i][j][0] - b[i][j][0]) * (result[i][j][0] - b[i][j][0])+
					(result[i][j][1] - b[i][j][1])*(result[i][j][1] - b[i][j][1])+
					(result[i][j][2] - b[i][j][2])*(result[i][j][2] - b[i][j][2]);
			tg1 = result[i][j][0] + result[i][j][1] +result[i][j][2];
			if(tg1 == 0){
				System.out.println("tg1_1 = " + tg1);
				rmse += (tg+1)/(tg1 + 1);
			}else
			//rmse += Math.pow(tg / tg1, 2);
			rmse += tg / tg1;
	...
	public static double[][] readMatrixU(String filename,int number_image){
		...
		try{
			...
			I = new double[(number_image-1)*Define.IMAGE_SIZE_ROW*Define.IMAGE_SIZE_COL][cluster]; // cluster
			int count = 0;
	...
	public static double[][] readMatrixU(String filename,int number_image,int Ci){
		...
		try{
			...
			//row = Integer.parseInt(inp.readLine());
			//column = Integer.parseInt(inp.readLine());
			I = new double[(number_image-1)*Define.IMAGE_SIZE_ROW*Define.IMAGE_SIZE_COL][Ci]; // cluster
			int count = 0;
			while ((st = inp.readLine()) != null) {
				line = new StringTokenizer(st);
				int t = 0;
